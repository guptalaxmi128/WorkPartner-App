...
import { useGetKYCQuery, useKycDetailsMutation } from "../services/signUpApi";
...
const KYC = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (permanentAddress && presentAddress && aadharImage && aadharNumber) {
      try {
        ...
        const res = await kycDetails(formData);
        alert("KYC details added successfully !");
        console.log(res);
        clearTextInput();
      } catch (error) {
        console.log(error);
  ...
  const onKYCPressed = () => {
    if (data) {
      navigation.navigate("UpdateKYC", { data });
    }
  };
